...
public class GraphLoader {
	...
	private static void addEdgesAndSegments(Collection<GeographicPoint> nodes,
		...
		for (GeographicPoint pt : nodes) {
			...
			for (RoadLineInfo info : outgoing) {
				...
				map.addEdge(pt, end, info.roadName, info.roadType, info.maxSpeed, length);
	...
	private static RoadLineInfo splitInputString(String input) {
		...
		return new RoadLineInfo(p1, p2, tokens.get(4), tokens.get(5),
				// Deal with the backward compatibility issues of older UCSD map format 
				tokens.size() == 7 ? Double.parseDouble(tokens.get(6)) : -1);
	...
	public static void main(String[] args) {
		...
		// GraphLoader.createIntersectionsFile("data/maps/toronto.map",
		// "data/intersections/toronto.intersections");
...
class RoadLineInfo {
	 ...
	 */
	RoadLineInfo(GeographicPoint p1, GeographicPoint p2, String roadName, String roadType, double maxSpeed) {
		point1 = p1;
		point2 = p2;
	...
	public RoadLineInfo getReverseCopy() {
		return new RoadLineInfo(point2, point1, roadName, roadType, maxSpeed);
	}
