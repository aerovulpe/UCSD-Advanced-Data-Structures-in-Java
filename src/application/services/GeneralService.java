...
public class GeneralService {
	...
	public float[] getBoundsArray() {
		...
		// [S, W, N, E]
		return new float[] { (float) sw.getLatitude(), (float) sw.getLongitude(), (float) ne.getLatitude(),
				(float) ne.getLongitude() };
	}
	...
	public void runFetchTask(String fName, ComboBox<DataSet> cb, Button button) {
		...
		task.setOnSucceeded(e -> {
			if (task.getValue().equals(fName)) {
				...
				try {
					Files.write(Paths.get(FetchController.PERSISTENT_PATH), (fName.substring(10) + "\n").getBytes(),
				...
				// System.out.println("Something went wrong, data not written to
				// file : Task succeeded but fName returned differently");
